...
Six architectures: FLNN [512,256,128] with focal loss ($\alpha=0.25$, $\gamma=2.0$), VAE (64-dim latent), 1D-CNN+Attention (4 heads), TabNet (3 steps), Stacked AE+Classifier [256,128,64], Transformer (8 heads, 3 layers). Mixed Precision (AMP) enabled 2× speedup. Achieved 82\% avg F1-Score.
...
\begin{tabular}{lcccc}
\toprule
\textbf{Model Category} & \textbf{F1-Macro} & \textbf{Bal. Acc} & \textbf{AUROC} & \textbf{Inference} \\
\midrule
BERT Models (4) & 88.0\% & 90.0\% & 0.94 & ~180ms \\
Traditional ML (12) & 85.0\% & 87.0\% & 0.91 & ~10ms \\
Deep Learning (6) & 82.0\% & 84.0\% & 0.88 & ~50ms \\
Hierarchical Transformer & 86.0\% & 88.0\% & 0.92 & ~200ms \\
Federated Contrastive & 84.0\% & 86.0\% & 0.90 & ~180ms \\
Meta-Learning & 81.0\% & 83.0\% & 0.87 & ~150ms \\
\bottomrule
\end{tabular}
\caption{Performance Comparison Across Model Categories}
\end{table}

\textbf{Top 6 Models:} XGBoost + SMOTE (88.5\% F1, 91.2\% Bal.Acc, 0.94 AUROC), DeBERTa-v3 + Focal Loss (88.0\%, 90.0\%, 0.94), LightGBM + BorderlineSMOTE (87.3\%, 90.8\%, 0.93), HLogFormer (86.0\%, 88.0\%, 0.92), Focal Loss NN (84.7\%, 88.3\%, 0.91), FedLogCL (84.0\%, 86.0\%, 0.90).

\textbf{Model Selection Frequency:} DeBERTa-v3: 7/16 (43.8\%), XGBoost: 6/16 (37.5\%), LogBERT: 5/16 (31.3\%), LightGBM: 4/16 (25.0\%).
...
\begin{itemize}
    \item \textbf{HealthApp (332:1)}: Baseline F1 18\% → ADASYN + VAE 72\% (+54\% improvement)
    \item \textbf{Spark (249:1)}: Baseline F1 15\% → BorderlineSMOTE + FLNN 68\% (+53\% improvement)
    \item \textbf{Android (73:1)}: Baseline F1 45\% → SMOTE + XGBoost 81\% (+36\% improvement)
\end{itemize}
...
\textbf{XGBoost + SMOTE} was selected for production deployment over DeBERTa-v3 (comparable 88\% accuracy) due to: (1) 18× faster inference (10ms vs 180ms), (2) CPU-only deployment, (3) interpretability (SHAP values), (4) smaller footprint (50MB vs 500MB). BERT models remain available for offline semantic analysis. Future work: model distillation, hybrid architectures, hardware acceleration (ONNX, TensorRT).
...
\section{References}
\begin{enumerate}
    \item He, P., et al. "Drain: An Online Log Parsing Approach with Fixed Depth Tree." \textit{IEEE International Conference on Web Services}, 2017.
    \item Devlin, J., et al. "BERT: Pre-training of Deep Bidirectional Transformers for Language Understanding." \textit{NAACL-HLT}, 2019.
    \item Chawla, N. V., et al. "SMOTE: Synthetic Minority Over-sampling Technique." \textit{Journal of Artificial Intelligence Research}, 2002.
    \item Lin, T. Y., et al. "Focal Loss for Dense Object Detection." \textit{IEEE International Conference on Computer Vision}, 2017.
    \item He, P., et al. "DeBERTa: Decoding-enhanced BERT with Disentangled Attention." \textit{ICLR}, 2021.
    \item Arik, S. Ö., \& Pfister, T. "TabNet: Attentive Interpretable Tabular Learning." \textit{AAAI}, 2021.
    \item Finn, C., et al. "Model-Agnostic Meta-Learning for Fast Adaptation of Deep Networks." \textit{ICML}, 2017.
